  ...
  render() {
    ...
    if(data!==undefined && page_init===0){
      ...
      for(let y=0;y<data_API.length;y++){
        ...
        if(data_API[y]!==undefined){
          for(let i=0;i<data_API[y].length;i++){
            lista_aux.push(parseFloat(data_API[y][i].EBITDAMargin))
      ...
      lista_margem_ebitda.map(function(item,i){
        if(lista_margem_ebitda[i].ano[0]<maior_ano){
          let diference = (maior_ano-lista_margem_ebitda[i].ano[0])
