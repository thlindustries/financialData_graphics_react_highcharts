  ...
  render() {
    ...
    if(data!==undefined && page_init===0){
      ...
      for(let y=0;y<data_API.length;y++){
        ...
        if(data_API[y]!==undefined){
          for(let i=0;i<data_API[y].length;i++){
            lista_aux.unshift(parseFloat(data_API[y][i].EBITDA))
      ...
      lista_ebitda.map(function(item,i){
        if(lista_ebitda[i].ano[lista_de_datas[i].date.length-1]>menor_ano){
          let diference = (lista_ebitda[i].ano[lista_de_datas[i].date.length-1]-menor_ano)
