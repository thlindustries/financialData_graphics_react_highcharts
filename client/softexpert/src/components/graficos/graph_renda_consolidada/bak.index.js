  ...
  render() {
    ...
    if(data!==undefined && page_init===0){
      ...
      for(let y=0;y<data_API.length;y++){
        ...
        if(data_API[y]!==undefined){
          for(let i=0;i<data_API[y].length;i++){
            lista_aux.push(parseFloat(data_API[y][i].ConsolidatedIncome))
        ...
        lista_renda_consolidada.push(obj);
      ...
      lista_renda_consolidada.map(function(item,i){
        if(lista_renda_consolidada[i].ano[0]<maior_ano){
          let diference = (maior_ano-lista_renda_consolidada[i].ano[0])
