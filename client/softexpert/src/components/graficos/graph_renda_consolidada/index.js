...
export default class GraphReceita extends Component {
  constructor(props) {
    ...
    axios.get('https://financialmodelingprep.com/api/v3/financials/income-statement/'+simbolo).then(resultado=>{
      this.setState({
        ...
        series: lista_renda_consolidada,
  ...
  render() {
    ...
    if(data!==undefined && page_init===0){
      ...
      for(let y=0;y<data_API.length;y++){
        ...
        if(data_API[y]!==undefined){
          for(let i=0;i<data_API[y].length;i++){
            lista_aux.unshift(parseFloat(data_API[y][i].ConsolidatedIncome))
        ...
        lista_renda_consolidada.unshift(obj);
      ...
      lista_renda_consolidada.map(function(item,i){
        if(lista_renda_consolidada[i].ano[lista_de_datas[i].date.length-1]>menor_ano){
          let diference = (lista_renda_consolidada[i].ano[lista_de_datas[i].date.length-1]-menor_ano)
          for(let y=0;y<diference;y++){
            lista_renda_consolidada[i].data.unshift(null)
          }
        }
      })
      //---------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        <Grid item xs={12}>
          <div>
            <HighchartsReact 
              ...
              options={{
                title: {
                  text: 'Renda consolidada das empresas '+todos_simbolos,
                },
