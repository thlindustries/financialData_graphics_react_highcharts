...
function createData(ConsolidatedIncome) {
...
export default class GraphMargemConsolidatedIncome extends Component {
  constructor(props) {
    ...
    axios.get('https://financialmodelingprep.com/api/v3/financials/income-statement/'+simbolo).then(resultado=>{
      this.setState({
        ...
        series: [
          {
            name:'Renda consolidada '+simbolo,
  ...
  render() {
    ...
    if(data!==undefined && page_init===0){
      ...
      data.map(function(item,i){
        data_API.push(createData(data[i].ConsolidatedIncome,))
      })
      page_init++;

      data_API.map(function(item,i){
        lista_renda_consolidada.push(parseFloat(data_API[i].ConsolidatedIncome))
      })
      // console.log(data)
    }
    return (
        ...
        <Grid item xs={12}>
          <div>
            <HighchartsReact 
              ...
              options={{
                title: {
                  text: 'Renda consolidada da empresa '+simbolo,
                },
